...
namespace Server.Dao
{
    class SubjectDao : GeneralDao
    {
        ...
        private void SetData(string[] request)
        {
            ...
            subjectId = request[2];
            date = request[3];
            ReplaceGroupNamesById();
        }

        private void ReplaceGroupNamesById()
        {
            List<string> groupsId = new List<string>();
            foreach (var k in this.groupsId)
            {
                groupsId.Add(SelectGroupIdByGroupName(k));
            }
            this.groupsId = groupsId.ToArray();
        }
        ...
        private DataTable SelectStudentsInGroup(string groupsId)
        {
